...
class Calendar extends Component {
  ...
  isDayInThisWeek = (week, markedDay) =>
    week.some(item => item.toString("yyyy MM dd") === markedDay);
  render() {
    const days = dateutils.page(this.state.currentMonth, this.props.firstDay);
    const weeks = [];
    const markedDay = Object.keys(this.props.markedDates)[0]
      ? parseDate(Object.keys(this.props.markedDates)[0]).toString("yyyy MM dd")
      : null;
    while (days.length) {
      const week = days.splice(0, 7);
      if (this.props.isCollapsed) {
        this.isDayInThisWeek(week, markedDay)
          ? weeks.push(this.renderWeek(week, weeks.length))
          : null;
      } else {
        weeks.push(this.renderWeek(week, weeks.length));
      }
    ...
    return (
      <View style={[this.style.container, this.props.style]}>
        <CalendarHeader
          theme={this.props.theme}
          hideArrows={this.props.isCollapsed}
